...
class SamlAuthenticator(Authenticator):
    ...
    session_cookie_names = Set(
        help='''
        Cookie names for managing SAML session that would be cleared upon logout
        ''',
        config=True
    )
    ...
    saml_settings_path = Unicode(
        ...
        config=True,
        help='''
        A filepath to the location of saml settings required for python3-saml. Contents of this directory
        should include settings.json, advanced_settings.json, and a cert/ directory if applicable
        '''
    )
